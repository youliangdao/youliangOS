...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, unsigned int memtotal)
{
  ...
  else if (cmdline[0] == 's' && cmdline[1] == 't' && cmdline[2] == 'a'
          && cmdline[3] == 'r' && cmdline[4] == 't' && cmdline[5] == ' ')
  {
    cmd_start(cons, cmdline, memtotal);
  }
  else if (cmdline[0] == 'n' && cmdline[1] == 'c' && cmdline[2] == 's'
          && cmdline[3] == 't' && cmdline[4] == ' ')
  {
    cmd_ncst(cons, cmdline, memtotal);
  }
  else if (cmdline[0] == 'l' && cmdline[1] == 'a' && cmdline[2] == 'n'
          && cmdline[3] == 'g' && cmdline[4] == 'm' && cmdline[5] == 'o' 
          && cmdline[6] == 'd' && cmdline[7] == 'e' && cmdline[8] == ' ')
  {
...
void cmd_hlt(struct CONSOLE *cons, int *fat)
{
  ...
  if (finfo != 0)
  {
    ...
    set_segmdsc(gdt + 1003, finfo->size - 1, (int)p, AR_CODE32_ER);
    farcall(0, 1003 * 8);
...
void cmd_start(struct CONSOLE *cons, char *cmdline, int memtotal)
{
  ...
  // コマンドラインに入力された文字列を一文字ずつ新しいコンソールに入力
...
void cmd_ncst(struct CONSOLE *cons, char *cmdline, int memtotal)
{
  ...
  int i;
  // コマンドラインに入力された文字列を一文字ずつ新しコンソールに入力
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
  ...
  if (finfo != 0)
  {
    ...
    if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00)
    {
      ...
      task->ds_base = (int)q;
      set_segmdsc(task->ldt + 0, finfo->size - 1, (int)p, AR_CODE32_ER + 0x60);
      set_segmdsc(task->ldt + 1, segsiz - 1, (int)q, AR_DATA32_RW + 0x60);
      for ( i = 0; i < datsiz; i++)
      {
        q[esp + i] = p[dathrb + i];
      }
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
  ...
  else if (edx == 5)
  {
    ...
    sheet_updown(sht, shtctl->top); // 今のマウスと同じ高さを指定。マウスはこの上に移動
    reg[7] = (int)sht;
  }
  else if (edx == 6)
  {
