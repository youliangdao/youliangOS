  ...
  *((int *)0x0fe8) = (int)nihongo;
  ...
  for (;;)
  {
    ...
    else
    {
      i = fifo32_get(&fifo);
      io_sti();
      if (key_win != 0 && key_win ->flags == 0)   // 入力ウィンドウが閉じられた
      {
      ...
      if (256 <= i && i <= 511) // キーボードデータ
      {
        ...
        if (i == 256 + 0x3b && key_shift != 0 && key_win != 0) // Shift + F1
        {
          ...
          if (task != 0 && task->tss.ss0 != 0)
          {
            ...
            io_sti();          
            task_run(task, -1, 0);  // 終了処理を確実にやらせるために寝ていたら起こす
          }
        }
        if (i == 256 + 0x39 && key_shift != 0)  // Shift + space
        {
          // 新しく作ったコンソールを入力状態にする
          if (key_win != 0)
          {
      ...
      else if (1024 <= i && i <= 2023)  // コンソールだけを閉じる
      {
...
void keywin_off(struct SHEET *key_win)
{
  ...
  {
    fifo32_put(&key_win->task->fifo, 2);  // コンソールのカーソルOFF
  }
  return;
}
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
  ...
  make_window8(buf, 256, 25, "console", 0);
