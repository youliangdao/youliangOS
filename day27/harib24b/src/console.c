...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, unsigned int memtotal)
{
  ...
  else if (strcmp(cmdline, "ls") == 0 && cons->sht != 0)
  {
    cmd_ls(cons);
		     ...
		     && cmdline[3] == 'e' && cmdline[4] == ' ' && cons->sht != 0)
  {
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
  ...
  if (finfo != 0)
  {
    ...
    if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00)
    {
      ...
      task->ds_base = (int)q;
      set_segmdsc(gdt + task->sel / 8 + 1000, finfo->size - 1, (int)p, AR_CODE32_ER + 0x60);
      set_segmdsc(gdt + task->sel / 8 + 2000, segsiz - 1, (int)q, AR_DATA32_RW + 0x60);
      for ( i = 0; i < datsiz; i++)
      {
        q[esp + i] = p[dathrb + i];
      }
