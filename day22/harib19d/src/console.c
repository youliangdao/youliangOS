...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
  ...
  if (finfo != 0)
  {
    ...
    *((int *)0xfe8) = (int)p;
    file_loadfile(finfo->clustno, finfo->size, p, fat, (char *)(ADR_DISKIMG + 0x003e00));
    set_segmdsc(gdt + 1003, finfo->size - 1, (int)p, AR_CODE32_ER + 0x60);
    set_segmdsc(gdt + 1004, 64 * 1024 - 1, (int)q, AR_DATA32_RW + 0x60);
    if (finfo->size >= 8 && strncmp(p + 4, "Hari", 4) == 0)
    {
      start_app(0x1b, 1003 * 8, 64 * 1024, 1004 * 8, &(task->tss.esp0));  // C言語ファイルで.hrb形式の場合
    }
    else
    {
      start_app(0, 1003 * 8, 64 * 1024, 1004 * 8, &(task->tss.esp0));     // アセンブリファイル
    }
    memman_free_4k(memman, (int)p, finfo->size);
